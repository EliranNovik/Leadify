...
async function runSQL() {
  try {
    console.log('🚀 Running stage and language foreign key setup...');
    
    // Read the SQL file
    const sqlPath = path.join(__dirname, 'add_stage_language_foreign_keys.sql');
...
async function checkExecSqlFunction() {
  try {
    ...
    if (error && error.message.includes('function exec_sql')) {
      ...
      console.log('📁 SQL file location:', path.join(__dirname, 'add_stage_language_foreign_keys.sql'));
      return false;
    }
    return true;
  } catch (err) {
...
async function main() {
  ...
  if (hasExecSql) {
    ...
    console.log('3. Run the contents of: add_stage_language_foreign_keys.sql');
  }
}
