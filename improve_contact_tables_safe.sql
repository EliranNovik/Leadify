-- Safe SQL Improvements for Contact Management Tables
-- This script adds foreign keys, constraints, and indexes for better functionality
-- Run cleanup_orphaned_contacts.sql FIRST to clean up orphaned records

-- 1. First, verify no orphaned records exist
DO $$
DECLARE
...
BEGIN
    ...
    IF orphaned_count > 0 THEN
        RAISE EXCEPTION 'Found % orphaned records in lead_leadcontact table. Please run cleanup_orphaned_contacts.sql first.', orphaned_count;
    END IF;
END $$;

-- 2. Verify main column has only valid values
DO $$
DECLARE
    invalid_main_count INTEGER;
BEGIN
    SELECT COUNT(*) INTO invalid_main_count
    FROM public.lead_leadcontact 
    WHERE main NOT IN ('true', 'false') AND main IS NOT NULL;
    
    IF invalid_main_count > 0 THEN
        RAISE EXCEPTION 'Found % records with invalid main column values. Please run fix_main_column_values.sql first.', invalid_main_count;
    END IF;
END $$;

-- 3. Add foreign key constraints to lead_leadcontact table
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE constraint_name = 'fk_lead_leadcontact_contact_id'
    ) THEN
...
-- Add foreign key to leads_lead table
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'fk_lead_leadcontact_lead_id'
    ) THEN
        ALTER TABLE public.lead_leadcontact 
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE constraint_name = 'chk_lead_leadcontact_main'
    ) THEN
        ALTER TABLE public.lead_leadcontact 
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
        WHERE constraint_name = 'chk_lead_leadcontact_not_both_null'
    ) THEN
        ALTER TABLE public.lead_leadcontact 
...
-- 5. Add useful indexes on leads_contact table
...
-- 6. Create a view for easier contact management
...
-- 7. Create a function to get all contacts for a lead
...
-- 8. Create a function to get the main contact for a lead
...
-- 11. Create a trigger to automatically update udate when contacts are modified
...
DROP TRIGGER IF EXISTS trg_update_contact_udate ON public.leads_contact;
CREATE TRIGGER trg_update_contact_udate
    BEFORE UPDATE ON public.leads_contact
    FOR EACH ROW
    EXECUTE FUNCTION update_contact_udate();

-- 12. Create a trigger to ensure only one main contact per lead
...
DROP TRIGGER IF EXISTS trg_ensure_single_main_contact ON public.lead_leadcontact;
...
SELECT 
    ...
    COUNT(*) as total_lead_leadcontact_records
FROM public.lead_leadcontact;
