...
const GenericCRUDManager: React.FC<GenericCRUDManagerProps> = ({
  ...
  const saveRecord = async (record: Partial<Record>) => {
    ...
    fields.forEach(field => {
      if (field.prepareValueForSave) {
        // Always apply prepareValueForSave, even if field is not in record (for hidden fields)
        if (field.name in record) {
          record[field.name] = field.prepareValueForSave(record[field.name], record);
        } else {
          // For hidden fields, apply prepareValueForSave and add to record
          record[field.name] = field.prepareValueForSave(undefined, record);
        }
      }
    });
    ...
    // Connected User is stored on users.employee_id — never on tenants_employee.user_id.
    let connectedUserId: string | null | undefined;
    let shouldSyncConnectedUser = false;
    if (tableName === 'tenants_employee' && 'connected_user_id' in record) {
      shouldSyncConnectedUser = true;
      const raw = record.connected_user_id;
      connectedUserId =
        raw === null || raw === undefined || raw === '' || raw === '---'
          ? null
          : String(raw);
      delete record.connected_user_id;
    }
    // Legacy field name from older forms — strip so it never hits the DB.
    if (tableName === 'tenants_employee' && 'user_id' in record) {
      if (!shouldSyncConnectedUser) {
        shouldSyncConnectedUser = true;
        const raw = record.user_id;
        connectedUserId =
          raw === null || raw === undefined || raw === '' || raw === '---'
            ? null
            : String(raw);
    ...
    try {
      ...
      if (existingRecordId) {
        ...
        if (tableName === 'users' && record.new_password && record.new_password.trim() !== '') {
          ...
          if (tableName === 'tenants_employee') {
            delete (updateData as any).connected_user_id;
        ...
        if (tableName === 'users') {
          ...
          // Also remove id for new records — we assign next max(id)+1 below (unless skipIdAssignment)
          ...
          delete finalInsertRecord.connected_user_id;
          delete finalInsertRecord.user_id;
          ...
          if (skipIdAssignment) {
            ...
            if (error) {
              ...
              if (conflictMessage) toast.error(conflictMessage);
              throw error;
            }
            result = data;
          } else {
            // Always use MAX(id)+1 with conflict retry — never fall back to id=1.
            try {
              result = await insertWithNextNumericId(tableName, finalInsertRecord);
              console.log(`✅ Inserted ${tableName} with allocated id:`, (result as any)?.id);
            } catch (error: any) {
      ...
      // Link Connected User via users.employee_id (create + edit)
      if (tableName === 'tenants_employee' && shouldSyncConnectedUser) {
        const employeeId = Number(existingRecordId ?? (result as any)?.id);
        if (Number.isFinite(employeeId) && employeeId > 0) {
          try {
            await syncEmployeeConnectedUser(employeeId, connectedUserId);
          } catch (linkError) {
            console.error('Error linking connected user to employee:', linkError);
            toast.error(
              'Employee saved, but linking the Connected User failed. You can edit the employee to retry.',
            );
          }
        }
      }
  ...
  const openModal = async (record?: Record) => {
    ...
    if (record && tableName === 'tenants_employee') {
      ...
      try {
        const linkedUserId = await fetchUserIdLinkedToEmployee(Number(record.id));
        (record as any).connected_user_id = linkedUserId || '';
      } catch (error) {
        console.error('Error fetching connected user for employee:', error);
        (record as any).connected_user_id = '';
      }
    }
