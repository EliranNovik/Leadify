...
const CommunicationsTab: React.FC<HandlerTabProps> = ({ leads }) => {
  ...
  }, [interactions.length, leads[0]?.id]); // Add leads[0]?.id dependency
  ...
  useEffect(() => {
    ...
    async function fetchAndCombineInteractions() {
      ...
      const manualInteractions = (leads[0]?.manual_interactions || []).filter((i: any) => i.kind !== 'whatsapp').map((i: any) => ({
        ...i,
        employee: i.direction === 'out' ? (userFullName || 'You') : i.employee || leads[0]?.name,
      }));
      ...
      if (leads[0]?.id) {
        try {
          ...
          if (emailsError) {
            ...
            clientEmails = emailsData || [];
            console.log('Fetched emails from database:', clientEmails.length);
          }
        } catch (fetchError) {
      ...
      const emailInteractions = clientEmails.map((e: any) => {
        ...
        return {
          ...
          raw_date: e.sent_at,
          employee: e.direction === 'outgoing' ? (userFullName || 'You') : leads[0]?.name,
      ...
      if (isMounted) {
        ...
        console.log('Updated emails state for modal:', formattedEmailsForModal.length);
      }
    }
    fetchAndCombineInteractions();
    return () => { isMounted = false; };
  }, [leads, currentUserFullName, whatsAppMessages.length, leads[0]?.id]);

  // This function now ONLY syncs with Graph and then triggers a full refresh
  const runGraphSync = useCallback(async () => {
    if (!leads[0]?.email || !instance || !accounts[0]) return;
    
    setEmailsLoading(true);

    try {
      const tokenResponse = await acquireToken(instance, accounts[0]);
      await syncClientEmails(tokenResponse.accessToken, leads[0]);
    } catch (e) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Use the pre-fetched current user information
      const senderName = currentUserFullName || 'Your Team';
      const userEmail = currentUserEmail || 'Me';
      ...
      // --- Save sent email to Supabase database ---
      const messageId = sentEmail.id || optimisticEmail.message_id;
      ...
      try {
                  const { data: savedEmail, error: dbError } = await supabase.from('emails').upsert([
            {
              message_id: messageId,
              client_id: leads[0].id,
        ...
        if (dbError) {
          ...
          toast.error('Email sent but failed to save to database.');
        } else {
      ...
      // If database save failed, try alternative approach
      if (!dbSaveSuccess) {
        console.log('Attempting alternative save method...');
        try {
          // Try with different field names in case the schema is different
          const { error: altError } = await supabase.from('emails').upsert([
            {
              message_id: messageId,
              lead_id: leads[0].id, // Try lead_id instead of client_id
              thread_id: sentEmail.conversationId || null,
              sender_name: senderName,
              sender_email: userEmail,
              recipient_list: leads[0].email,
          ...
          if (altError) {
            ...
            console.log('Email saved using alternative method');
            toast.success('Email sent and saved to database!');
          }
        } catch (altSaveError) {
  ...
  useEffect(() => {
    if (isEmailModalOpen) {
      const defaultSubject = `[${leads[0]?.lead_number}] - ${leads[0]?.name} - ${leads[0]?.topic || ''}`;
      setComposeSubject(prev => prev && prev.trim() ? prev : defaultSubject);
    }
  }, [isEmailModalOpen, leads]);
  ...
  const handleSaveContact = async () => {
    ...
    try {
      const existingInteractions = leads[0]?.manual_interactions || [];
        ...
        if (user && user.email) {
          ...
          if (!userError && userData) {
            if (userData.full_name && userData.full_name.trim()) {
              setCurrentUserFullName(userData.full_name.trim());
            } else if (userData.first_name && userData.last_name) {
              setCurrentUserFullName(`${userData.first_name.trim()} ${userData.last_name.trim()}`);
            } else {
              setCurrentUserFullName(user.email);
            }
          } else {
            setCurrentUserFullName(user.email);
          }
  ...
  useEffect(() => {
    if (isWhatsAppOpen) {
      console.log('WhatsApp leads[0]?.id:', leads[0]?.id);
      console.log('WhatsApp messages:', whatsAppMessages);
    }
  }, [isWhatsAppOpen, whatsAppMessages, leads[0]?.id]);
          ...
          <div className="bg-base-100 w-screen h-screen max-w-none max-h-none flex flex-col overflow-hidden">
            ...
            <div className="flex flex-col gap-3 p-3 md:p-4 border-b border-base-300 bg-white">
              <div className="flex items-center justify-between">
                <h3 className="text-lg md:text-xl font-bold truncate">Email Thread with {leads[0]?.name}</h3>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      const processedBody = template.body
                          .replace(/{client_name}/g, leads[0]?.name || '')
                          .replace(/{upload_link}/g, uploadLink);
                      const newSubject = `[${leads[0]?.lead_number}] - ${leads[0]?.name} - ${leads[0]?.topic || ''}`;
                    ...
                    >
                      <div className={`avatar placeholder ${email.direction === 'outgoing' ? 'hidden' : ''}`}>
                        <div className="bg-neutral-focus text-neutral-content rounded-full w-8 h-8 md:w-10 md:h-10">
                          <span className="text-xs md:text-sm">{leads[0]?.name?.charAt(0)}</span>
                        </div>
                      </div>
            ...
            >
              ...
              <div className="hidden md:block">
                <input type="text" className="input input-bordered w-full" value={leads[0]?.email} disabled />
              </div>
      ...
      {isWhatsAppOpen && createPortal(
        <div className="fixed inset-0 z-[999] flex flex-col bg-white w-full h-full max-w-none max-h-none">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-base-200 bg-white">
            <h3 className="text-xl font-bold">WhatsApp Chat with {leads[0]?.name}</h3>
          ...
          <div className="flex-1 overflow-y-auto flex flex-col w-full min-h-20 p-8" id="whatsapp-modal-messages">
            {whatsAppMessages.length === 0 ? (
              <div className="text-center p-8 text-base-content/70">
                ...
                <span className="text-xs text-error">(Debug: leads[0]?.id = {leads[0]?.id})</span>
              </div>
            ) : (
