...
import type { NineHourOvertimePhase } from '../hooks/useNineHourOvertimeAutomation';

interface NineHourOvertimeModalProps {
  isOpen: boolean;
  phase: NineHourOvertimePhase;
      ...
      >
          ...
          <div className="flex items-start gap-3 min-w-0">
            <span
              className={`inline-flex items-center justify-center w-10 h-10 rounded-full shrink-0 ${
                isFinalCountdown ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'
              }`}
            >
              {isFinalCountdown ? (
            ...
            <div className="min-w-0">
              <h3 id="nine-hour-overtime-title" className="text-lg font-semibold text-gray-900">
                {isFinalCountdown ? 'Clocking out soon' : '9 hours reached'}
              </h3>
        ...
        <div className="px-5 py-4 space-y-4">
          {isProcessing ? (
            ...
            <>
              <p className="text-sm text-gray-700 leading-relaxed">
                You have reached 9 hours of work today. Do you wish to continue working overtime?
              </p>
              <div className="rounded-xl bg-amber-50 border border-amber-200 px-4 py-3 text-center">
                <p className="text-xs font-semibold uppercase tracking-wide text-amber-800">
                  Respond within
                </p>
                <p className="text-3xl font-bold text-amber-900 tabular-nums mt-1">
