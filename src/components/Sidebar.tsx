...
const Sidebar: React.FC<SidebarProps> = ({ userName = 'John Doe', userInitials, isOpen = false, onClose }) => {
  ...
  return (
    <>
      ...
      <div className="fixed top-0 left-0 h-full flex flex-col items-center gap-3 py-4 z-40 bg-transparent mr-8 pr-2 ml-4 mt-16 hidden md:flex">
        {/* Navigation Items */}
        ...
        <div className="mt-4 flex flex-col items-center gap-1">
          <button
            className="sidebar-circle sidebar-circle-sm sidebar-noexpand group"
            title="Sign out"
            onClick={handleSignOut}
          >
            <span className="sidebar-icon-wrapper sidebar-icon-wrapper-sm">
              <ArrowRightOnRectangleIcon className="w-4 h-4 transition-colors duration-200 text-gray-700 group-hover:text-gray-900" />
            </span>
            <span className="sidebar-label">Sign Out</span>
          </button>
          <span className="text-xs text-base-content/70 mt-1 font-medium">{userName}</span>
        </div>
