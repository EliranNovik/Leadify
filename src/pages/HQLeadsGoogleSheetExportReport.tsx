...
const EXPORT_DESTINATION = 'hq_leads_capital_firm';
...
const HQLeadsGoogleSheetExportReport: React.FC = () => {
  ...
  const runSync = async (dryRun: boolean) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('google-sheets-hqleads-sync', {
      ...
      if (includeDebug && data != null) {
        console.info('[HQLeadsGoogleSheetExportReport] function response', data);
      }
      if (d?.error) {
        toast.error(d.error);
        return;
  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      ...
      <GoogleSheetAutoSyncNotice reportLabel="HQLeads" />
