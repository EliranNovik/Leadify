...
const MyCasesPage: React.FC = () => {
  ...
  const fetchMyCases = async () => {
    try {
      ...
      const nonActiveCasesList = allProcessedCases.filter(caseItem => {
        const stageId = (caseItem as any).stageId;
        const activeHandlerType = (caseItem as any).active_handler_type;
        return Number(activeHandlerType) === 1 && Number(stageId) !== 200;
      });
  ...
  const handleCaseClick = (caseItem: Case) => {
    // Client route for this page: always navigate by lead_number for NEW leads.
    // IMPORTANT: subleads include '/', so we must encode it (e.g. L209994/10 -> L209994%2F10)
    if (!caseItem) return;
    ...
    // Legacy leads use numeric ID (strip legacy_ if present)
  ...
  const handleRowSelect = (caseId: string, event?: React.MouseEvent) => {
    ...
    if (isNewTab) {
      ...
      if (caseItem) {
        const leadNumber = String(caseItem.lead_number || '');
        if (caseItem.isNewLead) {
          const url = `/clients/${encodeURIComponent(String(leadNumber).trim())}`;
          window.open(url, '_blank');
        } else {
          const legacyId = String(caseItem.id || '').replace(/^legacy_/, '');
          const url = leadNumber.includes('/')
            ? `/clients/${encodeURIComponent(legacyId)}?lead=${encodeURIComponent(leadNumber)}`
            : `/clients/${encodeURIComponent(legacyId)}`;
          window.open(url, '_blank');
        }
        return;
      }
  ...
  const handleToggleActiveHandler = async (caseItem: Case, newType: number, e: React.MouseEvent) => {
    ...
    try {
      ...
      toast.success(newType === 1 ? 'Transferred to Retention' : 'Case marked as active for you');
      await fetchMyCases();
    } catch (error: any) {
                ...
                >
                  <td className="px-3 sm:px-4 lg:px-6 py-2 sm:py-3 lg:py-4 min-w-[100px]">
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        type="button"
                        className="text-black font-medium text-xs sm:text-sm hover:underline text-left"
                        onClick={(e) => {
                          e.stopPropagation();
                  ...
                  <td className="px-3 sm:px-4 lg:px-6 py-2 sm:py-3 lg:py-4 min-w-[100px]">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      {/* Active -> Non-Active toggle (Transfer to Retention) */}
                      {isActiveCases && caseItem.active_handler_type === 2 && (
                        <button
                      ...
                      {!isActiveCases && !isNewCases && caseItem.active_handler_type === 1 && (
                        <button
