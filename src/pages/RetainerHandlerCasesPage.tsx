...
const RetainerHandlerCasesPage: React.FC = () => {
  ...
  useEffect(() => {
    if (user?.id) {
      fetchRetainerHandlerCases();
    }
  }, [user?.id]);

  // Fetch countries with timezone data (will be populated in fetchRetainerHandlerCases)
  ...
  const fetchRetainerHandlerCases = async () => {
    try {
      setLoading(true);
      setError(null);
      ...
      const [
        ...
        // Legacy leads: check retainer_handler_id (numeric)
        supabase
          .from('leads_lead')
          .select(`
            id,
      ...
      console.error('Error fetching retainer handler cases:', err);
  ...
  // Filter cases based on search query and category (no stage filter)
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <button
            onClick={fetchRetainerHandlerCases}
...
export default RetainerHandlerCasesPage;
