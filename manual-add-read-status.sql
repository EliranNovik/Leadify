-- Manual SQL script to add read status tracking to whatsapp_messages table
...
-- Update existing messages to be marked as unread (optional)
-- Uncomment the line below if you want to mark all existing messages as unread
-- UPDATE whatsapp_messages SET is_read = FALSE, read_at = NULL, read_by = NULL WHERE is_read IS NULL;
...
AND column_name IN ('is_read', 'read_at', 'read_by')
