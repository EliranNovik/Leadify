...
async function runSQL() {
  try {
    console.log('🔍 Checking stage column structure and creating foreign key...');
    
    // Read the SQL file
    const sqlPath = path.join(__dirname, 'check_and_fix_stage_column.sql');
...
async function checkExecSqlFunction() {
  try {
    ...
    if (error && error.message.includes('function exec_sql')) {
      ...
      console.log('📁 SQL file location:', path.join(__dirname, 'check_and_fix_stage_column.sql'));
      return false;
    }
    return true;
  } catch (err) {
...
async function main() {
  ...
  if (hasExecSql) {
    ...
    console.log('3. Run the contents of: check_and_fix_stage_column.sql');
  }
}
