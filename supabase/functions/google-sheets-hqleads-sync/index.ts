...
const LOG_PREFIX = '[google-sheets-hqleads-sync]';
...
function sheetRangeA1(tabName: string, cellRange: string): string {
  ...
  if (!t) throw new Error('Sheet tab name is empty; set GOOGLE_SHEET_HQLEADS_TAB');
...
serve(async (req) => {
  ...
  const { data: leads, error: rpcErr } = await admin.rpc('get_leads_for_hqleads_google_sheet_export', {
    p_limit: limit,
  });
  if (rpcErr) {
    logError('RPC get_leads_for_hqleads_google_sheet_export failed', rpcErr);
  ...
  const list = (leads ?? []) as Array<{
    ...
    conversion_value_nis: number | string | null;
  }>;

  log('RPC returned rows', { count: list.length });

  const spreadsheetId = Deno.env.get('GOOGLE_SHEET_HQLEADS_SPREADSHEET_ID') ?? DEFAULT_SPREADSHEET_ID;
  const tabName = Deno.env.get('GOOGLE_SHEET_HQLEADS_TAB')?.trim() || DEFAULT_SHEET_TAB;
  ...
  for (const l of list) {
    ...
    valueRows.push([gclid, 'HQLeads', convTime, conversionValue, 'ils']);
    logRows.push({
