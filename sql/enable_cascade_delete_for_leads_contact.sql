-- Enable CASCADE DELETE for all foreign keys that reference leads_contact table
-- This allows TRUNCATE CASCADE and individual row deletions to work properly

BEGIN;

-- ============================================
-- STEP 1: Find ALL foreign keys referencing leads_contact
-- ============================================
SELECT 
...
DECLARE
    ...
    referenced_col TEXT;
BEGIN
    -- Loop through all foreign keys referencing leads_contact
    FOR fk_record IN
        SELECT 
            tc.table_schema,
            tc.table_name AS referencing_table,
...
-- ============================================
-- STEP 3: Remove any duplicate constraints
-- ============================================
-- Check for and handle duplicates (similar to leads_lead issue)
DO $$
DECLARE
    ...
    table_col RECORD;
BEGIN
    -- Find tables with duplicate constraints on the same column
    FOR table_col IN
        SELECT 
            tc.table_name,
            kcu.column_name,
    ...
    LOOP
        ...
        -- Keep the first constraint (usually the standard one), drop the rest
...
-- ============================================
-- STEP 4: Verify all constraints now use CASCADE
-- ============================================
SELECT
    tc.table_name AS referencing_table,
    kcu.column_name AS referencing_column,
...
-- 1. Delete individual rows: DELETE FROM leads_contact WHERE id = 123;
-- 2. Truncate the table: TRUNCATE TABLE leads_contact CASCADE;
-- 3. Delete all rows: DELETE FROM leads_contact;
--
