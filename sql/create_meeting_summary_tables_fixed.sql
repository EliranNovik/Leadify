...
DO $$ 
BEGIN
    -- Drop meeting_transcripts policies
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'meeting_transcripts' AND policyname = 'Users can view meeting transcripts for their clients') THEN
        DROP POLICY "Users can view meeting transcripts for their clients" ON meeting_transcripts;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'meeting_transcripts' AND policyname = 'Service role can insert meeting transcripts') THEN
        DROP POLICY "Service role can insert meeting transcripts" ON meeting_transcripts;
    END IF;
    
    -- Drop meeting_summaries policies
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'meeting_summaries' AND policyname = 'Users can view meeting summaries for their clients') THEN
        DROP POLICY "Users can view meeting summaries for their clients" ON meeting_summaries;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'meeting_summaries' AND policyname = 'Service role can insert meeting summaries') THEN
        DROP POLICY "Service role can insert meeting summaries" ON meeting_summaries;
    END IF;
    
    -- Drop meeting_questionnaires policies
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'meeting_questionnaires' AND policyname = 'Users can view meeting questionnaires for their clients') THEN
        DROP POLICY "Users can view meeting questionnaires for their clients" ON meeting_questionnaires;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'meeting_questionnaires' AND policyname = 'Service role can insert meeting questionnaires') THEN
        DROP POLICY "Service role can insert meeting questionnaires" ON meeting_questionnaires;
    END IF;
END $$;

-- Create RLS policies for meeting_transcripts
...
-- Create RLS policies for meeting_summaries
...
-- Create RLS policies for meeting_questionnaires
...
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_meeting_transcripts_updated_at') THEN
        DROP TRIGGER update_meeting_transcripts_updated_at ON meeting_transcripts;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_meeting_summaries_updated_at') THEN
        DROP TRIGGER update_meeting_summaries_updated_at ON meeting_summaries;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_meeting_questionnaires_updated_at') THEN
        DROP TRIGGER update_meeting_questionnaires_updated_at ON meeting_questionnaires;
    END IF;
END $$;
