-- Portal documents: never block client uploads on CRM "documents needed" setup.
-- 1) Always return the full active document-type catalog (not per-lead assignments).
-- 2) Allow prepare-upload with a null document_type_id.
...
BEGIN
  ...
  -- Document type is optional. When provided, it must be an active catalog type
  -- (no per-lead assignment required).
