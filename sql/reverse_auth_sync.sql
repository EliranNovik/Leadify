-- Reverse auth sync system
-- Create users in auth system first, then automatically sync to users table
...
-- Function to create user with password (for admin panel)
...
BEGIN
    ...
    -- Create user in auth system using built-in function
