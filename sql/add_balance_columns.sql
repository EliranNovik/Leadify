...
ADD COLUMN IF NOT EXISTS balance_currency text DEFAULT '₪',
ADD COLUMN IF NOT EXISTS proposal_total decimal(10,2) DEFAULT 0.0,
ADD COLUMN IF NOT EXISTS proposal_currency text DEFAULT '₪',
...
COMMENT ON COLUMN leads.balance_currency IS 'Currency for the balance (₪, $, €, £)';
COMMENT ON COLUMN leads.proposal_total IS 'Total proposal amount';
COMMENT ON COLUMN leads.proposal_currency IS 'Currency for the proposal (₪, $, €, £)';
